...
private:
	...
	void createDescriptorSetLayout() {
		...
		samplerLayoutBinding.stageFlags = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT | VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
	...
	void createVertexBuffer() {
		VkDeviceSize bufferSize = sizeof(plane.vertices[0]) * plane.vertices.size();
		...
		memcpy(data, plane.vertices.data(), (size_t)bufferSize);
		vkUnmapMemory(device, stagingBufferMemory);
	...
	void createIndexBuffer() {
		VkDeviceSize bufferSize = sizeof(plane.indices[0]) * plane.indices.size();
		...
		void* data;
		vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, plane.indices.data(), (size_t)bufferSize);
		vkUnmapMemory(device, stagingBufferMemory);
	...
	void createCommandBuffers() {
		...
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			...
			vkCmdBeginRenderPass(commandBuffers[i], &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
				...
				vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(plane.indices.size()), 1, 0, 0, 0);
	...
	void updateUniformBuffer(uint32_t currentImage) {
		//Commenting out time code to get Camera working.
